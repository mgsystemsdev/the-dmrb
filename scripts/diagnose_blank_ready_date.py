...
def _main():
    ...
    # 1) Open turnovers (same criteria as board: closed_at IS NULL, canceled_at IS NULL, move_out_date IS NOT NULL)
    ...
    # 2) Find a unit that shows blank Ready Date: report_ready_date and available_date both NULL
    blank_ready = [
        t for t in open_list
        if t.get("report_ready_date") is None and t.get("available_date") is None
    ]
    if not blank_ready:
        # Fallback: pick first open turnover for reference
        chosen = open_list[0]
